...
export default function ExcelPage() {
  ...
  const exportMultipleExcel = async () => {
    ...
    const addSheets = async(id, sheet) => {
    ...
    week.forEach(async elem => {
        await addSheets("data-table"+elem.value, "Week"+elem.value)
    })
    // Save or serve the file as needed
    workbook.xlsx.writeBuffer().then((buffer) => {
      const blob = new Blob([buffer], { type: 'application/octet-stream' });
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `UserAttendance_${year.value}.xlsx`;
      link.click();
    });
    // alert(password)
  };
        ...
        <div  className={styles.DownloadActions}>
            ...
            <div className={styles.btns}>
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"  viewBox="0 0 16 16">
                    ...
                    <div className={styles.buttons} onClick={exportMultipleExcel}>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"  viewBox="0 0 16 16">
                      <path d="M8.5 6.5a.5.5 0 0 0-1 0v3.793L6.354 9.146a.5.5 0 1 0-.708.708l2 2a.5.5 0 0 0 .708 0l2-2a.5.5 0 0 0-.708-.708L8.5 10.293z"/>
                      <path d="M14 14V4.5L9.5 0H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2M9.5 3A1.5 1.5 0 0 0 11 4.5h2V14a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1h5.5z"/>
                    </svg>
...
{
  week.map((item, index) => (
    <table key={item.value}  id={"data-table" + item.value}  className={styles.visuallyHidden} >
    <TableHeader   week={item.value} position={sPosition.value} year={year.value}/> 
  </table>
  ))
}
