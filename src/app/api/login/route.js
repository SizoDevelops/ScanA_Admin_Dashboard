...
export async function POST(request) {
    ...
    if (user.count > 0 && (await bcrypt.compare(body.password, user.items[0].password))) {
        ...
        const { password, members, attendance, school_address,school_code,school_email,school_number, school_slogan,coordinates,school_admin, posts, school_meetings, user_faces,  ...userWithoutPass } = user.items[0];
