  ...
  reducers: {
    ...
    updateMember: (state, action) => {
      if (state.value.members.find((elem) => elem.id === action.payload.id)) {
        let member = state.value.members.find(
        ...
        for (const key in member) {
          ...
          for (const items in action.payload) {
            ...
            if (item !== i) {
              let lem = state.value.members.filter(
    ...
    uploadMeetings: (state, action) => {
      if (
        !state.value.school_meetings.find(
          (elem) => elem.id === action.payload.id
        )
      ) {
    ...
    deleteMeeting: (state, action) => {
      state.value.school_meetings.filter(
        (items) => items.id !== action.payload.id
      );
    },
    addMovementCode: (state, action) => {
      state.value.movementCodes.push(action.payload);
    },
    removeMovementCode: (state, action) => {
      state.value.movementCodes = state.value.movementCodes.filter(
